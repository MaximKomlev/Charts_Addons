...
open class AggregatedBarChartRenderer: BarChartRenderer {
    ...
    public func findDataEntryAt(x: CGFloat, y: CGFloat) -> (dataSetIndex: Int, dataEntryIndex: Int)? {
        ...
        if (idx > -1) {
            let groupRects = self.groupRects[idx]
    ...
    public func findDataIndexFor(highlight: Highlight) -> Int {
        guard
            let dataProvider = dataProvider,
            let barData = dataProvider.barData
            else { return -1 }
        ...
        guard
            let set = barData.getDataSetByIndex(highlight.dataSetIndex) as? IBarChartDataSet,
            set.isHighlightEnabled
            else { return -1 }
        
        let trans = dataProvider.getTransformer(forAxis: set.axisDependency)
        let pt = trans.pixelForValues(x: highlight.x, y: 0)
        let idx = self.findBarForPoint(x: pt.x)
        return idx != -1 ? self.groupRects[idx].dataIndexOfTopEntry : -1
    }

    public func barPointFor(highlight: Highlight) -> CGPoint {
        guard
            let dataProvider = dataProvider,
            let barData = dataProvider.barData
        ...
        let pt = trans.pixelForValues(x: highlight.x, y: 0)
        let idx = self.findBarForPoint(x: pt.x)
        return idx != -1 ? self.groupRects[idx].rect.origin : CGPoint.zero
    }

    public func barColorFor(highlight: Highlight) -> UIColor {
        guard
            let dataProvider = dataProvider,
            let barData = dataProvider.barData
        ...
        let pt = trans.pixelForValues(x: highlight.x, y: 0)
        let idx = self.findBarForPoint(x: pt.x)
        return idx != -1 ? self.groupRects[idx].color : UIColor.clear
    }
